...
echo "Deploy QT"
windeployqt.exe --no-translations --no-system-d3d-compiler --no-opengl-sw --no-opengl --no-svg --no-webkit --no-patchqt --no-webkit2 --release  %deployPath%\QUsb2Snes.exe
...
mkdir %deployPath%\apps\Magic2Snes
xcopy %usb2snes%\apps %deployPath%\apps /sy
xcopy %magic2snes% %deployPath%\apps\Magic2Snes /syq
