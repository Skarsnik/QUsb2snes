...
if [ $DO_BUILD = 1 ]; then
    ...
    $QMAKE_EXEC -makefile DEFINES+="SQPROJECT_INSTALLED" DEFINES+="SQPROJECT_UNIX_INSTALL_PREFIX=\\\\\\\"$COMPILE_PREFIX\\\\\\\"" DEFINES+="SQPROJECT_UNIX_APP_SHARE=\\\\\\\"$APPLICATION_COMPILE_SHARE\\\\\\\"" ../QFile2Snes.pro $@
    if [ $SKIP_MAKE = 0 ]; then
        make -j 4
    fi
    cd -
